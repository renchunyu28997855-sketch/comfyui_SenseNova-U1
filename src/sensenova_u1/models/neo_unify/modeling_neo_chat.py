    ...
    def _generate_think(
        ...
        for i in range(max_think_tokens):
            ...
            if token_item == eos_token_id:
                logger.info(f"[think] break on eos at step {i}")
                break
            if token_item == think_end_token_id:
                logger.info(f"[think] break on think_end at step {i}, token={token_item}")
            ...
            if (i + 1) % 50 == 0:
                logger.info(f"[think] step {i + 1}/1024, next_token={next_token.item()}, t_idx={t_idx}")

        logger.info(f"[think] done, generated {len(think_token_ids)} tokens")
